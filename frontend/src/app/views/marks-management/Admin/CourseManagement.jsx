...
const CourseModal = ({ isOpen, onClose, onSave, course = null, schemes = [] }) => {
    const [formData, setFormData] = useState({ 
        code: '', name: '', semester: 1, credits: 3, scheme: '' 
    });

    useEffect(() => {
        if (course) {
            setFormData({ code: course.code, name: course.name, semester: course.semester, credits: course.credits, scheme: course.scheme || '' });
        } else {
            setFormData({ code: '', name: '', semester: 1, credits: 3, scheme: schemes.length > 0 ? schemes[0].id : '' });
        }
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input required type="text" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value.toUpperCase()})} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm font-mono uppercase" placeholder="e.g. 18CS51" disabled={!!course} />
                        </div>
                        <div>
                            ...
                            <input required type="number" min="1" max="10" value={formData.credits} onChange={e => setFormData({...formData, credits: parseInt(e.target.value)})} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm" placeholder="e.g. Machine Learning" />
                    </div>
                    ...
                    <div>
                        ...
                        <select required value={formData.scheme} onChange={e => setFormData({...formData, scheme: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm">
                            <option value="">-- Select Scheme --</option>
                            {schemes.map(s => <option key={s.id} value={s.id}>{s.name} ({s.id})</option>)}
                        </select>
                    ...
                    <div>
                        ...
                        <select value={formData.semester} onChange={e => setFormData({...formData, semester: parseInt(e.target.value)})} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm">
                    ...
                    <div className="flex justify-end space-x-3 mt-6 pt-2 border-t dark:border-gray-700">
                        ...
                        <button type="submit" className="px-4 py-2 text-sm font-bold text-white bg-primary-600 rounded-md hover:bg-primary-700 shadow-sm transition-colors">Save Course</button>
                    </div>
                </form>
            </div>
        </div>
...
const CourseManagement = () => {
    ...
    const openAddModal = () => { setSelectedCourse(null); setIsModalOpen(true); };
    const openEditModal = (course) => { setSelectedCourse(course); setIsModalOpen(true); };
    ...
    const handleSave = async (formData) => {
        ...
        toast.promise(
            selectedCourse ? api.patch(`/courses/${selectedCourse.id}/`, payload) : api.post('/courses/', payload),
            {
                ...
                error: 'Failed to save. Ensure Course Code is unique.',
            }
        ).then(() => {
            closeModal();
            fetchData();
    ...
    return (
        <div className="p-6 space-y-6">
            <CourseModal isOpen={isModalOpen} onClose={closeModal} onSave={handleSave} course={selectedCourse} schemes={schemes} />
                                    ...
                                    {courses.length === 0 ? (
                                        ...
                                        courses.map(c => (
                                            <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-500 dark:text-gray-400">{c.semester}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium text-gray-500 dark:text-gray-400">{c.credits}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div className="flex justify-end gap-3">
                                                        <button onClick={() => openEditModal(c)} className="text-primary-600 hover:text-primary-900 dark:text-primary-400 transition-colors"><Icons.PencilSquare className="w-5 h-5" /></button>
                                                        <button onClick={() => openDeleteModal(c.id)} className="text-red-600 hover:text-red-900 dark:text-red-400 transition-colors"><Icons.Trash className="w-5 h-5" /></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))
